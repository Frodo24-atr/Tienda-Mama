...
const conditionLabel: Record<string, string> = {
  nuevo: 'Nuevo', 'como-nuevo': 'Como nuevo', 'muy-buen-estado': 'Muy buen estado', 'buen-estado': 'Buen estado',
};
const conditionColor: Record<string, string> = {
  nuevo: 'bg-emerald-100 text-emerald-800', 'como-nuevo': 'bg-sky-100 text-sky-800',
  'muy-buen-estado': 'bg-amber-100 text-amber-800', 'buen-estado': 'bg-stone-100 text-stone-700',
};
    ...
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-stone-100">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-lg font-bold text-stone-900">${product.price.toLocaleString('es-AR')}</p>
          <button onClick={() => !inCart && addToCart(product)} disabled={inCart}
            className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-xl transition-all duration-200 ${
              inCart ? 'bg-emerald-50 text-emerald-700 cursor-default' : 'bg-stone-900 text-white hover:bg-amber-700'
            }`}>
            {inCart ? <><Check size={13} />En carrito</> : <><ShoppingBag size={13} />Agregar</>}
          </button>
        </div>
      </div>
    </div>
